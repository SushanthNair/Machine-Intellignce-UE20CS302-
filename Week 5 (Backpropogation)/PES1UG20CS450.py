...
class Tensor:
    ...
    def grad_add(self, gradients=None):
        ...
        """
        # TODO
        if gradients is None:           #No gradient, so gradient is 1
            grad1 = np.ones(self.shape)
            grad2 = np.ones(self.shape)
        else:                           #Store gradient in 2 variables
            grad1 = gradients
            grad2 = gradients

        if self.history[1].requires_grad == True:   #add gradients from previous node and update history
    ...
    def grad_matmul(self, gradients=None):
        ...
        """
        # TODO
        if gradients == None:               #No gradient, so gradient is 1
            Grad = np.ones(self.shape)
        else:
            Grad = gradients
        grad1 = np.matmul(Grad, np.transpose(self.history[2].arr))  #Dot product of Grad and previous gradient
    ...
    def backward(self, gradients=None):
        ...
        """
        if self.history[0] != 'leaf':           #If node is a leaf node, don't do any operation
            ...
            self.history[1].backward(g1)    #Repeat until leaf nodes are reached
